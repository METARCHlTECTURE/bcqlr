...
| **主客观**   | 动名词更具`客观性`，表示动作作为一种中性活动，没有感情色彩。<br>例：Learning English is important.（学习英语很重要。——表示学习的客观活动）                                                                                                                                    | 不定式更具`主观性`，表达说话者的主观意图、目的或未来打算。<br>例：To learn English is my goal.（学习英语是我的目标。——表示主观意图）                                                                                                                                                                                   |
...
4. **大小（Size）**：如 *big, small, tiny*
5. **形状（Shape）**：如 *round, square, triangular*
6. **年龄（Age）**：如 *old, young, new*
7. **颜色（Color）**：如 *red, blue, green*
...
“**of + 名词**” 结构在英语中非常常见，它通常起到形容词性修饰作用，用来进一步描述名词的性质、类别、来源或状态。
...
### 表示悲伤或失望（Sadness/Disappointment）
- **Oh no!**（哦，不！）
- **Alas!**（唉！[表示悲伤或遗憾]）
- **Oh dear!**（天哪！）
- **Boo-hoo!**（呜呜呜！[表示哭泣声]）
...
### 表示庆祝或鼓励（Encouragement/Celebration）
- **Cheers!**（干杯！）
- **Well done!**（做得好！）
- **Good job!**（干得好！）
- **Way to go!**（好样的！）
...
### 表示思考或沉思（Thinking/Contemplation）
- **Hmm…**（嗯……）
- **Uh…**（呃……）
- **Er…**（呃……）
- **Well…**（嗯……）
...
### 表示肯定或否定（Agreement/Disagreement）
- **Yes!**（是！）
- **Yeah!**（是的！）
- **Yup!**（嗯！）
- **Sure!**（当然！）
...
### 其他常用感叹词（Miscellaneous Interjections）
- **Oops!**（哎呀！）
- **Aha!**（啊哈！）
- **Phew!**（呼！）
- **Tut-tut!**（啧啧！）
...
| 数字          | 英文                                | 数字       | 英文         | 数字          | 英文                 |
| ------------- | ----------------------------------- | ---------- | ------------ | ------------- | -------------------- |
| **0**         | zero British:nought 电话号码年份:oh | **10**     | ten          | **20**        | twenty               |
...
| **123**       | one hundred and twenty-three        | **1000**   | one thousand | **100,000**   | one hundred thousand |
| **1,000,000** | one million                         | $$ 10^9 $$ | one billion  | $$ 10^{12} $$ | one trillion         |

基础数字的特点：
- 21到99之间的复合数字用连字符（hyphen）连接，如：**21 (twenty-one), 45 (forty-five), 99 (ninety-nine)**。
- 大于100的数字，如：**123** 表达为 "**one hundred and twenty-three**"。英式英语通常在“百”与“十”之间使用 **and**，而美式英语通常省略该连接词。
- “Hundred”、“thousand”后不加复数形式（不加 -s）：
  - 正确：two hundred people
  - 错误：two hundreds people
- 英语中表示数字时，逗号用来分隔每三位数，如：**1,234,567**，即"one million, two hundred thirty-four thousand, five hundred sixty-seven"。
...
| $$ 10^9 $$    | one billion                         | one billionth                          |
| $$ 10^{12} $$ | one trillion                        | one trillionth                         |

规则：
- 大多数序数词在基数词后加上 **-th**，如 **fourth, sixth, seventh**。
- 特殊变化包括：
  - **1st (first)**，**2nd (second)**，**3rd (third)**，**5th (fifth)**，**9th (ninth)**，**12th (twelfth)**。
- 复合数字如 21, 42, 73 等的序数词，只将最后一位基数词变为序数形式：
  - **21st**: twenty-first
  - **42nd**: forty-second
  - **73rd**: seventy-third
- 千及以上的数字：任何大于1000的数字，都遵循相同规则：
  - **1,234**: one thousand two hundred and thirty-fourth
  - **100,000**: one hundred thousandth

使用场景：
- 在表示顺序、日期时常用序数词：
  - "I came **second** in the race."（我在比赛中获得了第二名。）
  - "Today is the **twenty-third** of March."（今天是三月二十三日。）

### 分数（Fractions）
分数用来表示部分数量或比例。分数的读法与序数词密切相关。

基本规则：
- 分母大于1时（如1/2, 2/3, 3/4），分母用序数形式，分子用基数形式：
...
特殊用法：
- 整数与分数结合时，如 **2 1/2**，读作："**two and a half**"。
- 其他常用表达：
  - **3/4**: three-quarters
  - **5/8**: five-eighths

### 小数（Decimals）
小数的表示方法通常使用“点”（point），如 **3.14** 读作 "**three point one four**"。
...
2. **小数金额**：数字加货币单位：
   ...
   - **£6.20**: six pounds and twenty pence
